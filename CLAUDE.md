...
- [Dockerfile](Dockerfile) — multi-stage build.
  ...
  - **Runtime**: `debian:bookworm-slim` with `ca-certificates` + `libssl3` only. Runs as non-root user `app` (uid 10001), listens on `0.0.0.0:8080`. `CMD` is `/app/server` — `dx bundle --platform web --release` (Dioxus 0.7.x) writes the server binary named literally **`server`** at the bundle root, with static assets in a sibling `public/` directory. An earlier draft assumed `/app/server/agent_maker` (binary named after the crate, in a `server/` subdir); that produced `stat /app/server/agent_maker: not a directory` because `/app/server` was actually the binary file. The Dockerfile now asserts `test -x "$OUT/server"` during the build so a layout mismatch fails at build time, not run time.
